...
pub(crate) const WLAN_AUTH_LEAP: u16 = 128;

/// A parsed or buildable IEEE 802.11 LEAP Authentication management frame
/// (algorithm 128).
#[derive(Debug, PartialEq, Eq, Clone)]
#[non_exhaustive]
pub struct Ieee80211AuthFrameLeap {
    /// Frame Control.
...
impl Ieee80211AuthFrameLeap {
    /// Create a LEAP Authentication frame in the STA-to-AP direction.
    pub fn new(
        sta_mac: [u8; 6],
        bssid: [u8; 6],
        transaction: u16,
    ...
    /// Parse a full IEEE 802.11 LEAP Authentication management frame.
    pub fn parse(data: &[u8]) -> Result<Self, DecodeError> {
        let (fixed, payload) = parse_auth_frame(data, Some(WLAN_AUTH_LEAP))?;
...
impl Emitable for Ieee80211AuthFrameLeap {

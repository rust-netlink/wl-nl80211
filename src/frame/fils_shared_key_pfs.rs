...
/// `WLAN_AUTH_FILS_SK_PFS` (5): FILS Shared Key authentication with PFS.
pub(crate) const WLAN_AUTH_FILS_SK_PFS: u16 = 5;

/// A parsed or buildable IEEE 802.11 FILS Shared Key Authentication frame
/// with PFS (IEEE Std 802.11-2024 Table 9-71: algorithm 5).
#[derive(Debug, PartialEq, Eq, Clone)]
#[non_exhaustive]
pub struct Ieee80211AuthFrameFilsSharedKeyPfs {
    /// Frame Control.
...
impl Ieee80211AuthFrameFilsSharedKeyPfs {
    /// Create a FILS Shared Key with PFS Authentication frame in the
    /// STA-to-AP direction.
    pub fn new(
        sta_mac: [u8; 6],
        bssid: [u8; 6],
    ...
    /// Parse a full IEEE 802.11 FILS Shared Key with PFS Authentication
    /// management frame.
    pub fn parse(data: &[u8]) -> Result<Self, DecodeError> {
        let (fixed, payload) =
            parse_auth_frame(data, Some(WLAN_AUTH_FILS_SK_PFS))?;
...
impl Emitable for Ieee80211AuthFrameFilsSharedKeyPfs {

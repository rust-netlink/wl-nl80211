...
/// `WLAN_AUTH_FILS_SK` (4): FILS Shared Key authentication without PFS.
pub(crate) const WLAN_AUTH_FILS_SK: u16 = 4;

/// A parsed or buildable IEEE 802.11 FILS Shared Key Authentication frame
/// without PFS (IEEE Std 802.11-2024 Table 9-71: algorithm 4).
#[derive(Debug, PartialEq, Eq, Clone)]
#[non_exhaustive]
pub struct Ieee80211AuthFrameFilsSharedKey {
    /// Frame Control.
...
impl Ieee80211AuthFrameFilsSharedKey {
    /// Create a FILS Shared Key Authentication frame in the STA-to-AP
    /// direction.
    pub fn new(
        sta_mac: [u8; 6],
        bssid: [u8; 6],
    ...
    /// Parse a full IEEE 802.11 FILS Shared Key Authentication management
    /// frame.
    pub fn parse(data: &[u8]) -> Result<Self, DecodeError> {
        let (fixed, payload) = parse_auth_frame(data, Some(WLAN_AUTH_FILS_SK))?;
...
impl Emitable for Ieee80211AuthFrameFilsSharedKey {

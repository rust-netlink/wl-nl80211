...
pub(crate) const WLAN_AUTH_FILS_PK: u16 = 6;

/// A parsed or buildable IEEE 802.11 FILS Public Key Authentication
/// management frame (IEEE Std 802.11-2024 Table 9-71: algorithm 6).
#[derive(Debug, PartialEq, Eq, Clone)]
#[non_exhaustive]
pub struct Ieee80211AuthFrameFilsPublicKey {
    /// Frame Control.
...
impl Ieee80211AuthFrameFilsPublicKey {
    /// Create a FILS Public Key Authentication frame in the STA-to-AP
    /// direction.
    pub fn new(
        sta_mac: [u8; 6],
        bssid: [u8; 6],
    ...
    /// Parse a full IEEE 802.11 FILS Public Key Authentication management
    /// frame.
    pub fn parse(data: &[u8]) -> Result<Self, DecodeError> {
        let (fixed, payload) = parse_auth_frame(data, Some(WLAN_AUTH_FILS_PK))?;
...
impl Emitable for Ieee80211AuthFrameFilsPublicKey {

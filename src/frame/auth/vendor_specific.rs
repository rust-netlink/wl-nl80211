...
/// `WLAN_AUTH_VENDOR_SPECIFIC` (65 535): vendor specific use.
pub(crate) const WLAN_AUTH_VENDOR_SPECIFIC: u16 = 65_535;

/// A parsed or buildable vendor specific Authentication management frame
/// (algorithm 65 535).
#[derive(Debug, PartialEq, Eq, Clone)]
#[non_exhaustive]
pub struct Ieee80211AuthFrameVendorSpecific {
    /// Frame Control.
...
impl Ieee80211AuthFrameVendorSpecific {
    /// Create a vendor specific Authentication frame in the STA-to-AP
    /// direction.
    pub fn new(
        sta_mac: [u8; 6],
        bssid: [u8; 6],
    ...
    /// Parse a full IEEE 802.11 vendor specific Authentication management
    /// frame.
    pub fn parse(data: &[u8]) -> Result<Self, DecodeError> {
        let (fixed, payload) =
            parse_auth_frame(data, Some(WLAN_AUTH_VENDOR_SPECIFIC))?;
    ...
    fn fixed(&self) -> Ieee80211AuthFrameFixed {
        Ieee80211AuthFrameFixed {
            ...
            auth_alg: WLAN_AUTH_VENDOR_SPECIFIC,
...
impl Emitable for Ieee80211AuthFrameVendorSpecific {

...
/// `WLAN_AUTH_PASN` (7): PASN authentication.
pub(crate) const WLAN_AUTH_PASN: u16 = 7;

/// A parsed or buildable IEEE 802.11 PASN Authentication management frame
/// (IEEE Std 802.11-2024 Table 9-71: algorithm 7).
#[derive(Debug, PartialEq, Eq, Clone)]
#[non_exhaustive]
pub struct Ieee80211AuthFramePasn {
    /// Frame Control.
...
impl Ieee80211AuthFramePasn {
    /// Create a PASN Authentication frame in the STA-to-AP direction.
    pub fn new(
        sta_mac: [u8; 6],
        bssid: [u8; 6],
        transaction: u16,
    ...
    /// Parse a full IEEE 802.11 PASN Authentication management frame.
    pub fn parse(data: &[u8]) -> Result<Self, DecodeError> {
        let (fixed, payload) = parse_auth_frame(data, Some(WLAN_AUTH_PASN))?;
...
impl Emitable for Ieee80211AuthFramePasn {

...
impl Nla for Nl80211BssInfo {
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ...
            Self::SignalMbm(d) => NativeEndian::write_i32(buffer, *d),
            Self::BeaconTsf(d) => NativeEndian::write_u64(buffer, *d),
            Self::InformationElements(_) => todo!(),
...
impl Nl80211InformationElements {
    fn parse_vec<T: AsRef<[u8]> + ?Sized>(
        ...
        while offset < payload.len() {
            // Note: it is possible to get an out of bound panic
            // somewhere in that loop. I don't remember how I got
            // it so I can't reproduce it, but it is possible
